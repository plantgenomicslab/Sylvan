...
foreach my $gene_ID ( sort keys %gene_info ) {
    ...
    foreach my $mRNA_ID ( @{$gene_info{$gene_ID}{"mRNA_ID"}} ) {
        ...
        my ($mRNA_start, $mRNA_end, $mRNA_content) = &get_mRNA_info($mRNA_CDSs, $mRNA_exons, $mRNA_ID, $mRNA_header[0], $strand, \%gff3_score_attr);
...
sub get_mRNA_info {
    ...
    # 对mRNA内容进行排序
    my $mRNA_content = &sort_mRNA_info($chr, $strand, $mRNA_ID, $mRNA_start, $mRNA_end, $mRNA_CDSs, $mRNA_exons,$mRNA_introns, $mRNA_UTR, \%gff3_score_attr);
...
sub sort_mRNA_info {
    ...
    if ( $strand eq "+" ) {
        my $num = 0;
        foreach ( split /\n/, $mRNA_exons ) {
            $num ++; @_ = split /\t/, $_; my $feature_type = "exon"; my $feature_showname = $feature_type;
            my $out_content = &get_content($chr, $feature_type, $_[0], $_[1], $strand, $mRNA_ID, $num, $feature_showname, \%gff3_score_attr);
        ...
        my $num = 0;
        foreach ( split /\n/, $mRNA_introns ) {
            $num ++; @_ = split /\t/, $_; my $feature_type = "intron"; my $feature_showname = $feature_type;
            my $out_content = &get_content($chr, $feature_type, $_[0], $_[1], $strand, $mRNA_ID, $num, $feature_showname, \%gff3_score_attr);
            $sort1{$out_content} = $_[0];
            $sort2{$out_content} = 4;
        }
        ...
        foreach ( split /\n/, $mRNA_UTR ) {
            ...
            my $out_content = &get_content($chr, $feature_type, $_[1], $_[2], $strand, $mRNA_ID, $num, $feature_showname, \%gff3_score_attr);
...
sub modify_stop {
    ...
    @CDS = sort {$a <=> $b} @CDS;
    my @CDS_orig = @CDS;  # Save original CDS for restoration on failure
    my $fill_ok = 0;
    ...
    if ( $fill_ok == 0 ) {
        ...
        @CDS = @CDS_orig;  # Restore original CDS on failure
    }
...
sub get_content {
    my ($chr, $feature_type, $start, $end, $strand, $mRNA_ID, $num, $feature_showname) = @_;
    my %gff3_score_attr = %{$_[-1]};
    my $score = '.';
    my $add_attr = "";
    ...
    my $out_content = "$chr\t\.\t$feature_type\t$start\t$end\t$score\t$strand\t.\tID=$mRNA_ID.$feature_showname$num;Parent=$mRNA_ID;$add_attr\n";
    return $out_content;
}
