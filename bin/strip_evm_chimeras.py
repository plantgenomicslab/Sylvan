...
## 🔴 이 가드는 주석에서 모델을 지우지 않는다 (2026-07-31 실측)

이름 때문에 오해하기 쉬우므로 명시한다. **효과는 PASA 입력에만 미친다.**

`PREFILTER` 를 만드는 `rule agat_clean_final` 은 이 스크립트의 출력이 아니라
**원본 `EVM/EVM.all.gff3`** 를 읽는다. 그 병합 단계(`merge_pasa_evm.py`)는
"PASA 가 덮지 않은 EVM 유전자"를 구조하는데, 여기서 뺀 모델은 PASA 에 넘기지
않았으니 **정의상 미덮임 → 전부 구조**된다.

Sly `PREFILTER` 실측: 제거한 57 개가 전부 되돌아왔고 EVM 원본과 **좌표 57/57
일치**, 파일 최대값도 그 키메라들의 값 그대로다(669 exon, 42,592 aa).

역할 분담은 이렇게 읽어야 한다:

    이 가드   PASA 가 파싱 못 하는 입력에 죽는 것을 막는다.
              Sly 는 이 가드 전 두 번 실패(7h22m, 4h45m), 후 17h37m 완주.
              런이 산출물을 내려면 반드시 필요하다.
    RF filter 키메라를 주석에서 실제로 제거한다. 이들은 애매한 사례가 아니다 —
              Sly 57 개는 exon 염기 기준 반복서열 88.2~100%(중앙값 98.9%)로
              `repeat_cov` 0.5 를 크게 넘고, Spe_A 의 동종 모델은
              TPM 0 · BLAST 없음 · Pfam 없음 · HELIXER 0 · REPEAT 0.721 로
              실제로 버려졌다.

즉 키메라는 **반복서열 밀집 구간의 EVM 산물**이고, 초안까지는 살아남으며,
